...
* Shared preferences can store very small amount of data only whereas SQLite has the ability
to store huge amounts of data
* Therefore, in our application we will be using the SQLite database to store the favorite
tracks*/
...
* These queries are pre-defined statements similar to english statements which
perform any action in the database*/
...
* SongPath STRING);" */
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {TODO("not implemented") //To change body of created functions use File | Settings | File Templates.
    }
...
/*Here we define the Content Values which store the values which are pushed into
the database*/
...
/*Here we use the insert function to insert the values into the table whose name is
specified using the TABLE_NAME
* and the values which are added are the content values*/
        db.insert(Staticated.TABLE_NAME, null, contentValues)
/*After performing the db actions, we are required to close the database in-order
to prevent any data leakage and saving the resources*/
        db.close()
    }
